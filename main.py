...
class Board(Widget):
    ...
    def normalize(self, x, y):
        # Returns a tuple of normalized x, y values.
        x = int(round(x))
        y = int(round(y))
        return x, y
    ...
    def can_move(self, board_x, board_y):
        return (self.valid_cell(int(board_x), int(board_y)) and self.b[int(board_x)][int(board_y)] is None)
    ...
    def new_tile(self, *args):
        ...
        if len(empty_cells) == 1 and self.is_deadlocked():
            print('Gane Over(board is deadlocked)')
        self.moving = False
